...
*   
*/
class MarketingAPI_Controller  extends CI_Controller
{
  public $request;
  public $login_user;
  ...
  function verifyToken($token)
  {
        ...
        if($check)
        {
          ...
          else
          {
            ...
            if($checkPlan && $checkPlan->expire_date>now){
        				return true;
        		}else{
        				//echo "<pre>";print_R($checkPlan->id);die;
        				$updata  =array('status'=>'4','updated_on'=>now);  //updating plan to expire
        				$this->RestModal->updateData('merchant_subscription',$updata,array('id'=>$checkPlan->id));
        				//echo $this->db->last_query();die;
        				$res=array('status'=>'408','message'=>'Plan expired.');
                $this->response($res);
        		}    
